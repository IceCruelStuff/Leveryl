...
class Server {
	...
	public function __construct(\ClassLoader $autoloader, \ThreadedLogger $logger, $filePath, $dataPath, $pluginPath, $defaultLang = "unknown"){
		...
		try{
			...
			if(file_exists($this->filePath . "src/pocketmine/resources/leveryl_$lang.yml")){
				...
				$content = file_get_contents($file = $this->filePath . "src/pocketmine/resources/leveryl_eng.yml");
			}
			...
			$this->advancedConfig = new Config($this->dataPath . "leveryl.yml", Config::YAML, []);
	...
	public function start(){
		...
		if(!file_exists($this->getPluginPath() . DIRECTORY_SEPARATOR . "Leveryl")){
			@mkdir($this->getPluginPath() . DIRECTORY_SEPARATOR . "Leveryl");
		}
